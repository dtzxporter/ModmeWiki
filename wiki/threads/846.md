...
foreach( trig in level.pack_a_punch.triggers )
	trig SetVisibleToTeam( "allies" );
{% endhighlight %}
<br /></blockquote><p style="text-align:left;">Thanks, would this also work for the mystery box, This is what i have currently but wont work</p>{% highlight php %}
...
<p>Reply By: natesmithzombies<br /><blockquote><em>ZoekMeMaar</em><p style="text-align:left;">I tested the function for the pack a punch but it only works when power is not active, if the power is active it will just say 2500 to upgrade</p></blockquote><p style="text-align:left;">A bit of a hackish way to get around it, but it will work: </p>{% highlight php %}
foreach( trig in level.pack_a_punch.triggers )
