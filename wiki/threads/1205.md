...
level.round_number{% endhighlight %}
 <br />that should give you the round number. It also might help to use waittills such as:<br />{% highlight php %}
level waittill( "start_of_round" );
//or
level waittill( "end_of_round" );{% endhighlight %}
</p>
...
<p>Reply By: modric<br />I have this and nothing happens. I want it to spawn in a model and trigger and also play a sound when the round is reached. The model and shootable trigger are shot and then the shootable trig opens a secret door. <br />{% highlight php %}
function distant()
{
...
<p>Reply By: mathfag<br /><blockquote><em>modric</em><br />mathfag whle(1) { level waittill(&quot;between_round_over&quot;); if(level.round_number == 50) { do stuff } }  To be honest I don&#39;t know what your change does exactly. Should I just run function distant() and then this?</blockquote><br /> <br />{% highlight php %}
...
}//start again{% endhighlight %}
 <br />the change is that before it only checked once now it does it forever</p>

---
<strong style="font-size: 1.4em;"><span style="text-decoration: underline;text-decoration-color: #34a7f9;"><span style="color:#34a7f9;">ModmeBot</span></span>:</strong>

<p>Reply By: modric<br />Since you already helped me with a shootable script, I figured I could use that one and combine with this for the actual reward part of the distant easter egg. I&#39;m sure I screwed something up when combining them but let me know if this would work<br />{% highlight php %}
...
level.trig_total = tony_trigs.size;{% endhighlight %}
 <br /> <br />instantly<br /> <br />tony_trigs - not defined<br /> <br />and its not like you just mis typed &quot;trig&quot; either - coz that would still be an error - as it isnt an array<br /><br /><br />{% highlight php %}
hide_model(){% endhighlight %}
 <br />doesnt exist - and seemingly would do nothing<br /> <br />{% highlight php %}
while(1) //repeats forever{% endhighlight %}
 <br /> <br />syntax error... no parenthesis...<br /><br /><br />{% highlight php %}
if(level.round_number == 5) //if the round number is 50{% endhighlight %}
 <br /> <br />syntax error... no parenthesis... - again ( and thats also round 5 - not 50... )<br /> <br />{% highlight php %}
level.tonytrigs_current++;{% endhighlight %}
 <br /> <br />another error - level.tonytrigs_current is not defined, so that is<br /><br />&quot;undefined&quot; + 1<br /> <br /> which also means that...<br /> <br />{% highlight php %}
IPrintLnBold("Collected tony "+level.tonytrigs_current+"/"+level.tonytrigs_total);	{% endhighlight %}
 <br /> <br />is also yet another error... ( infact, even better - its &quot;two&quot; errors in one line )<br /><br />and<br /><br /><br />{% highlight php %}
tony_reward();{% endhighlight %}
...
<p>Reply By: modric<br />Thank you for your help Harry with the syntax stuff, I tried to sort out the syntax errors, but still feel the <br />while (1) //repeats forever<br />level waittill(&quot;between_round_over&quot;); //when round changes this runs<br /> if level.round_number == 5 stuff needs parenthesis. I am more concerned with the concept of the script, that everything lines up, since this is 2 scripts mixed that include some concepts I am not so familiar with. I am going to test it soon assuming the syntax is cleaned up.<br />{% highlight php %}
function tony_distant()
{
...
trig = GetEnt("distant_trig", "targetname");
	...
	level.trig_total = trigs.size;{% endhighlight %}
 <br /> <br />still have an error immediatly<br /> <br />trig - is not an array<br /> <br />{% highlight php %}
while (1) //repeats forever{% endhighlight %}
 <br /> <br />you still havent added a parenthesis here<br /> <br />{% highlight php %}
if level.round_number == 5 //if the round number is 50{% endhighlight %}
 <br /> <br />5 still isnt 50<br /> <br />{% highlight php %}
hide_model2(); //calling the hdie_model() function{% endhighlight %}
 <br /> <br />is running on &quot;level&quot; not the trig - level has no &quot;target&quot;<br /> <br />{% highlight php %}
level.trigs_current++;	{% endhighlight %}
...
<p>Reply By: Harry Bo21<br /><blockquote><em>modric</em><br />yea sorry i left some syntax in there i kind of rushed that, its all cleaned up now. </blockquote><br /> dude your not even paying attention to what im saying<br /> <br />{% highlight php %}
trig = GetEnt("distant_trig", "targetname");

	level.trig_total = trigs.size;{% endhighlight %}
 <br /> <br /><strong>&quot;TRIG&quot; IS NOT AN <span style="color:#ff0000;">ARRAY</span></strong><br /> <br /> <br />for the third flipping time<br /> <br /> <br />{% highlight php %}
if level.round_number == 5 //if the round number is 50{% endhighlight %}
 <br /> <br />and that is EVEN WORSE and STILL not round 50<br /><br /><br />{% highlight php %}
while (1) //repeats forever{% endhighlight %}
...
<p>Reply By: mathfag<br /> <br /> <img style="max-width: 500px;" src="http://i0.kym-cdn.com/photos/images/original/001/233/131/e09.jpg"><br />{% highlight php %}
function tony_distant()
{
...
<p>Reply By: Harry Bo21<br />almost<br /> <br />{% highlight php %}
model = GetEnt( self.target, "targetname" );{% endhighlight %}
 <br /> <br /> <br />self == level</p>

---
<strong style="font-size: 1.4em;"><span style="text-decoration: underline;text-decoration-color: #34a7f9;"><span style="color:#34a7f9;">ModmeBot</span></span>:</strong>

<p>Reply By: mathfag<br /><blockquote><em>Harry Bo21</em><br />almost   model = GetEnt( self.target, &quot;targetname&quot; );       self == level</blockquote><br /> Ye that line used to be in a new function<br />{% highlight php %}
model = GetEnt( trig.target, "targetname" );{% endhighlight %}
</p>

---
<strong style="font-size: 1.4em;"><span style="text-decoration: underline;text-decoration-color: #34a7f9;"><span style="color:#34a7f9;">ModmeBot</span></span>:</strong>

<p>Reply By: modric<br />I actually made the script nearly from scratch before you posted your new one mathfag, obviously I am not very good at scripting stuff but let me know if I&#39;m on the right track. I&#39;ll use yours for my map since its guaranteed to be better, but mine is a start I think. <br />{% highlight php %}
...
while (1) 
	...
	level waittill("between_round_over"); 
	if (level.round_number == 5) {% endhighlight %}
 <br /> <br />are you ever gonna learn to use the {} ffs....<br /> <br />{% highlight php %}
function show_distant()
{
 ...
 <br />&quot;trig&quot; AND &quot;model&quot; are both undeclared variables within this functions space too...<br /> <br />{% highlight php %}
function shoot_distant()
{	
	self waittill( "damage", amount, attacker, direction_vec, point, type, tagName, modelName, partName, weapon, dFlags, inflictor, chargeLevel );
	if(weapon == GetWeapon("bo3_m1911"))
 ...
 <br />same again here... and &quot;trig&quot; is &quot;self&quot; here anyway...<br /> <br /> <br />{% highlight php %}
show_distant(){% endhighlight %}
