...
jblundell_quote = "fog rolling in";{% endhighlight %}
 <br /> <br />now because we&#39;ve stored the string inside that variable, we can use that variable in place of where we would normally use a string. for example in the <span style="color:#ffcc00;">IPrintLnBold()</span> function:<br />{% highlight php %}
jblundell_quote = "fog rolling in";
IPrintLnBold(jblundell_quote);{% endhighlight %}
 <br /> <br />note this is the same as:<br />{% highlight php %}
IPrintLnBold("fog rolling in");{% endhighlight %}
 <br /> <br /> <br /><span style="color:#00ffff;">NUMBERS</span><br /> <br />Just like what you learned in Algebra, numbers can be letters too! yay!<br /> <br />let&#39;s define a variable as a number:<br /> <br />{% highlight php %}
push_ups_performed = 0;{% endhighlight %}
 <br /> <br />if we want to do math operations on a variable, we can do it like this:<br /> <br />{% highlight php %}
...
push_ups_performed = push_ups_performed / 3; //divides by 3{% endhighlight %}
 <br />notice we have to type it twice, because we want to define it as itself (whatever it used to be) plus the 3, not just as 3.<br /> <br />there is shorthand however for simply adding 1 to it or subtracting 1 from it:<br />{% highlight php %}
push_ups_performed ++; //adds 1
push_ups_performed --; //subtracts 1{% endhighlight %}
 <br /> <br />POP QUIZ: What number will this print to the screen?<br />{% highlight php %}
...
watermelon_model = GetEnt("watermelon","targetname");{% endhighlight %}
...
function calculate_bmi( weight, height )
{
 ...
 <br /> <br />In this example the function is asking for two <span style="color:#ffcc00;">parameters</span>: a weight, and a height.<br /> <br />it then calculates bmi by dividing the variable weight by height, and then dividing it by height again.<br /> <br />because I used the word weight and the word height as my parameters, any arguments that get passed to this function will be stored in a variable called weight and a variable called height, which I use for the calculation.<br /> <br />so if I wanted to use this function, I could do this:<br /> <br />{% highlight php %}
calculate_bmi( 70, 1.8 ); //weight is in kg and height is in m {% endhighlight %}
...
function calculate_bmi( weight, height )
{
 ...
 <br /> <br />it&#39;s as simple as putting the word return before the value we want to return. Now when calling this function, in order to receive the bmi value we must set a variable for it to be stored in:<br /> <br />{% highlight php %}
my_bmi = calculate_bmi( 60, 1.5);{% endhighlight %}
